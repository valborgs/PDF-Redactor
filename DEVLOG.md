...
- **PDF 백업 기능 추가**: 마스킹 저장 전 원본 PDF 자동 백업 기능 구현 완료
...
## 2025-11-18: PDF 백업 기능 구현 (5단계)

### 1. 개요

마스킹 작업 시 원본 PDF 파일을 자동으로 백업하여 데이터 손실을 방지하는 기능을 구현했습니다.
...
- 백업 폴더: 프로젝트 최상단에 `backup` 폴더 생성
- 일자별 하위 폴더: `YYYYMMDD` 형식 (예: `20251118`)
- 파일명: 원본 파일명 그대로 유지

#### 2.2 주요 메서드

**`backup_current_pdf() -> tuple[bool, str]`**
...
**기능:**
- 현재 열린 PDF 파일을 프로젝트 최상단의 backup 폴더로 복사
- 일자별 폴더 자동 생성 (`os.makedirs(backup_dir, exist_ok=True)`)
- 프로젝트 루트 경로: `os.path.dirname(os.path.abspath(__file__))`
- 이미 백업된 파일이 있으면 스킵 (중복 백업 방지)
- 성공 시: `(True, 백업_경로)` 반환
- 실패 시: `(False, 오류_메시지)` 반환
...
**기능:**
- 설정 메뉴에서 백업 옵션을 켜고 끌 수 있음
- 현재 상태를 `self.backup_enabled`에 저장
- 상태 변경 시 안내 메시지 표시
...
**특징:**
- 체크 가능한 메뉴 항목 (`setCheckable(True)`)
- 기본값: 활성화 (`self.backup_enabled = True`)
- 실시간 토글 가능
...
2. "마스킹을 저장하시겠습니까?" 확인
   ↓
3. [예] 선택 시 백업 수행 (백업 활성화된 경우)
   ↓
4. 백업 성공 → 마스킹 적용 및 저장
   ↓
5. 백업 실패 → "백업 없이 계속 진행?" 확인
   ├─ [예] → 백업 없이 저장
   └─ [아니요] → 저장 취소
```
...
```python
success_msg = f"총 {len(self.masks)}개의 마스킹이 적용되어 저장되었습니다."
if self.backup_enabled:
    success_msg += "\n\n원본 파일은 '백업' 폴더에 저장되었습니다."
```

### 3. 주요 라이브러리

```python
import shutil  # 파일 복사 (copy2)
from datetime import datetime  # 일자 생성
```

**`shutil.copy2(src, dst)`**
- 파일 복사 + 메타데이터 보존 (생성일, 수정일 등)
- 파일 권한도 함께 복사
...
3. Ctrl+S → "저장하시겠습니까?" → [예]
4. 자동 백업 수행
   - backup/20251118/파일명.pdf 생성 (프로젝트 최상단)
5. 마스킹 적용 및 저장
```
...
```
1. 백업 폴더 생성 권한 없음 또는 디스크 공간 부족
2. "백업 실패: (오류 메시지)" 다이얼로그 표시
3. "백업 없이 계속 진행하시겠습니까?"
   - [예] → 저장 진행
   - [아니요] → 저장 취소
```
...
**특징:**
- 예외 발생 시에도 프로그램 종료되지 않음
- 사용자에게 선택권 제공 (백업 없이 진행 또는 취소)
...
**장점:**
- 같은 날 같은 파일을 여러 번 저장해도 백업은 한 번만
- 디스크 공간 절약
- 백업 속도 향상
...
└── contracts/
    ...
    │       ├── contract_002.pdf (원본)
    │       └── contract_003.pdf (원본)
    ├── contract_001.pdf (마스킹 적용됨)
    ├── contract_002.pdf (마스킹 적용됨)
    └── contract_003.pdf (마스킹 적용됨)
```
...
✅ 백업 폴더 자동 생성 (`백업/YYYYMMDD/`)  
✅ 파일 복사 (메타데이터 보존)  
✅ 설정 메뉴에 백업 활성화/비활성화 옵션  
✅ 저장 전 자동 백업 수행  
✅ 백업 실패 시 선택적 진행  
✅ 중복 백업 방지  
✅ 성공 메시지에 백업 정보 표시  
...
- 🔒 **데이터 안전성**: 원본 파일 보존
- ⚙️ **유연한 설정**: 메뉴에서 쉽게 활성화/비활성화
- 🚨 **에러 핸들링**: 백업 실패 시에도 작업 계속 가능
- 💾 **공간 최적화**: 중복 백업 방지
- 📅 **일자별 관리**: 날짜별로 백업 파일 정리
...
1. **기본 백업 테스트**
   ...
   - 마스킹 영역 선택
   - Ctrl+S → 저장
   - `백업/YYYYMMDD/` 폴더에 원본 파일 생성 확인

2. **백업 비활성화 테스트**
   - 설정 > "PDF 백업 활성화" 체크 해제
   - 마스킹 저장
   - 백업 폴더에 파일이 생성되지 않음 확인
...
4. **백업 실패 처리 테스트**
   ...
   - 저장 시도
   - "백업 실패" 다이얼로그 표시 확인
   - "백업 없이 계속 진행" 선택 가능 확인

5. **폴더 구조 확인**
   - 백업 폴더 위치가 PDF와 같은 폴더인지 확인
   - 일자별 하위 폴더 생성 확인
   - 파일명이 원본과 동일한지 확인
...
## 2025-11-18: 인증 기능 구현 (6단계)

### 1. 개요

프로그램 사용을 위한 시리얼 번호 기반 라이선스 인증 시스템을 구현했습니다.

### 2. 구현 내용

#### 2.1 LicenseManager 클래스

라이선스 관리를 담당하는 핵심 클래스입니다.
...
- 내용:
  ```json
  {
    ...
    "serial_hash": "...",
    "activated_at": "2025-11-18T10:30:00"
  }
  ```

#### 2.2 SerialInputDialog 클래스

시리얼 번호 입력을 위한 모달 다이얼로그입니다.

**UI 구성:**
- 안내 문구
- 시리얼 번호 입력 필드 (형식: XXXX-XXXX-XXXX-XXXX)
- 인증 버튼
- 취소 버튼
- 상태 메시지 표시
...
1. 프로그램 시작
   ↓
2. 라이선스 파일(.license) 확인
   ↓
3. [라이선스 없음] → 시리얼 번호 입력 다이얼로그 표시
   ↓
4. 사용자가 시리얼 번호 입력
   ↓
5. [인증] 버튼 클릭
...
def activate_license(self, serial: str) -> tuple[bool, str]:
    try:
        response = requests.post(
            'https://license-server.com/api/activate',
            json={
                'serial': serial,
...
```
1. 프로그램 실행
2. "라이선스 인증" 다이얼로그 자동 표시
3. 시리얼 번호 입력: TEST-1234-5678-ABCD
4. [인증] 버튼 클릭
5. "라이선스 서버에 연결 중..." 프로그레스 표시
6. "라이선스가 성공적으로 활성화되었습니다." 메시지
7. 메인 윈도우 표시
```

#### 3.2 두 번째 실행 이후 (라이선스 있음)
...
```
1. 잘못된 시리얼 번호 입력: ABCD-1234-5678-9999
2. [인증] 버튼 클릭
3. "유효하지 않은 시리얼 번호입니다." 에러 메시지
4. 다이얼로그 유지 → 재시도 가능
```
...
```
1. [취소] 버튼 클릭
2. "라이선스 인증이 필요합니다. 프로그램을 종료합니다." 메시지
3. 프로그램 종료
```

### 4. 테스트용 시리얼 번호

개발 및 테스트를 위해 다음 시리얼 번호를 사용할 수 있습니다:
...
```python
입력: "1234"
결과: "올바른 형식이 아닙니다. (XXXX-XXXX-XXXX-XXXX)"

입력: "ABC-1234-5678-9999"
결과: "올바른 형식이 아닙니다. (XXXX-XXXX-XXXX-XXXX)"
```
...
```python
입력: "INVALID-SERIAL-NUMBER"
결과: "유효하지 않은 시리얼 번호입니다."
```
...
✅ LicenseManager 클래스 구현  
✅ SerialInputDialog UI 구현  
✅ 시리얼 번호 형식 검증  
✅ 라이선스 파일 저장/로드  
✅ 프로그램 시작 시 자동 인증 확인  
✅ 인증 성공/실패 메시지 표시  
...
1. **시리얼 번호 암호화**: 평문 대신 암호화하여 저장
2. **머신 바인딩**: 특정 컴퓨터에 라이선스 바인딩
3. **만료일 체크**: 라이선스 유효 기간 설정
4. **주기적 검증**: 프로그램 실행 중 주기적으로 재검증
5. **서버 인증**: 실제 라이선스 서버와 통신
...
2. **유효한 시리얼 테스트**
   - 시리얼 입력: `TEST-1234-5678-ABCD`
   - [인증] 클릭
   - 성공 메시지 확인
   - 메인 윈도우 표시 확인

3. **무효한 시리얼 테스트**
   - 시리얼 입력: `INVALID-SERIAL-HERE`
   - [인증] 클릭
   - 실패 메시지 확인
   - 다이얼로그 유지 확인
